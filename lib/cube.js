...
Cube.toFace = function () {
    ...
    var length = 256;
    canvas.width = length * 7;
    canvas.height = length;
    var context = canvas.getContext("2d");
    if(context){
        var addFace = function (color, offset) {
            context.fillStyle = Color.BLACK;
            context.fillRect(offset, 0, length, length);
            context.fillStyle = color;
            context.fillRect(offset+16, 16, length-16*2, length-16*2);
            context.strokeStyle = color;
            context.lineJoin = "round";
            context.lineWidth = 16;
            context.strokeRect(offset+16, 16, length-16*2, length-16*2);
        };
        addFace(Color.RED, 0);
        addFace(Color.ORANGE, length);
        addFace(Color.BLUE, length*2);
        addFace(Color.GREEN, length*3);
        addFace(Color.YELLOW, length*4);
        addFace(Color.WHITE, length*5);
        addFace(Color.BLACK, length*6);
        return canvas;
    } else {
